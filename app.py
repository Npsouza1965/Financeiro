...
# ✅ CORREÇÃO: Import correto dos módulos
...
def set_background_static(relative_path):
    path = resource_path("c:\\NPS-FIN\\modulos\\static\\tela_fundo_azul.png")
    if os.path.exists(path):
        with open(path, "rb") as f:
            data = f.read()
        encoded = base64.b64encode(data).decode()
...
def main_app():
    ...
    if not st.session_state["authenticated"]:
        set_login_background("c:\\NPS-FIN\\modulos\\Fundo.png")
        usuarios.login_screen()  # ✅ Agora vai funcionar!
        return
    ...
    if menu == "Financeiro":
        ...
        relacionamento.formulario_relacionamento()
        set_background_static("c:\\projetos\\static\\tela_fundo_azul.png")    
    elif menu == "Planos":
...
# ✅ Apenas esta linha deve chamar main_app()
main_app()