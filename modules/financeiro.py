...
def inicializar_estado_financeiro():
    ...
    estado_padrao = {
        ...
        "financeiro_classificacao": "Crédito",  # CORREÇÃO: Campo classificação adicionado
...
def limpar_campos_financeiro():
    ...
    campos_para_limpar = [
        ...
        "financeiro_plano_selecionado", "financeiro_classificacao",  # CORREÇÃO: Incluir classificação
        "financeiro_limpar_campos"
    ]
    ...
    for campo in campos_para_limpar:
        ...
        elif campo == "financeiro_classificacao":
            # CORREÇÃO: Define classificação baseada no tipo atual
            st.session_state[campo] = "Crédito" if st.session_state["financeiro_tipo_selecionado"] == "Entrada" else "Débito"
        elif campo == "financeiro_limpar_campos":
...
def exibir_financeiro():
    ...
    with col1:
        ...
        if tipo_selecionado != tipo_anterior:
            ...
            # CORREÇÃO: Atualizar classificação automaticamente
    ...
    # CORREÇÃO: Classificação automática baseada no tipo
    ...
    if not df_fin.empty:
        ...
        if not df_filtrado_periodo.empty:
            ...
            # CORREÇÃO: Preencher automaticamente quando um registro é selecionado
            if escolha and escolha != st.session_state["financeiro_ultima_escolha"]:
                ...
                # CORREÇÃO: Preencher TODOS os campos com dados do registro selecionado
                ...
                # CORREÇÃO: Preencher classificação do registro
    ...
    # CORREÇÃO: Formulário de classificação com valores atualizados
    st.subheader("📋 Classificação da Transação")
    
    col1, col2, col3, col4, col5 = st.columns(5)  # CORREÇÃO: 5 colunas para incluir Classificação
    ...
    # CORREÇÃO: Campo Classificação (somente leitura)
    with col4:
    ...
    if btn_salvar or btn_alterar or btn_excluir or btn_limpar:
        ...
        dados_transacao = {
            ...
            "classificacao": st.session_state["financeiro_classificacao"],  # CORREÇÃO: Incluir classificação
    ...
    # CORREÇÃO: Limpar campos após ação
    if st.session_state["financeiro_limpar_campos"]:
